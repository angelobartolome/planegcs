...
namespace GCS
{
    ...
    double ConstraintSlopeAtBSplineKnot::grad(double *param)
    {
        ...
        for (size_t i = 0; i < numpoles; ++i)
        {
            ...
            if (param == weightat(i))
            {
                ...
                result = (factors[i] * xslopesum - slopefactors[i] * xsum) * diry - (factors[i] * yslopesum - slopefactors[i] * ysum) * dirx;
                return scale * result;
            }
        }
    ...
    {
        if (*theparam() < bsp.flattenedknots[startpole + bsp.degree] || *theparam() > bsp.flattenedknots[startpole + bsp.degree + 1])
        {
    ...
    double ConstraintPointOnBSpline::grad(double *gcsparam)
    {
        ...
        if (gcsparam == theparam())
        {
            ...
            {
                d[i - 1] = (*poleat(i) * *weightat(i) - *poleat(i - 1) * *weightat(i - 1)) / (bsp.flattenedknots[startpole + i + bsp.degree] - bsp.flattenedknots[startpole + i]);
            }
            ...
            for (size_t i = 1; i < numpoints; ++i)
            {
                d[i - 1] = (*weightat(i) - *weightat(i - 1)) / (bsp.flattenedknots[startpole + i + bsp.degree] - bsp.flattenedknots[startpole + i]);
            }
    ...
    {
        double deriv = 0.;
        if (param == l1p1x() || param == l1p1y() || param == l1p2x() || param == l1p2y() || param == l2p1x() || param == l2p1y() || param == l2p2x() || param == l2p2y())
        {
            double x0 = ((*l1p1x()) + (*l1p2x())) / 2;
            double y0 = ((*l1p1y()) + (*l1p2y())) / 2;
            double x1 = *l2p1x(), x2 = *l2p2x();
    ...
    {
        double deriv = 0.;
        if (param == c1x() || param == c1y() || param == c2x() || param == c2y() || param == r1() || param == r2())
        {
            double dx = (*c1x() - *c2x());
            double dy = (*c1y() - *c2y());
            if (param == c1x())
    ...
    double ConstraintPointOnEllipse::error()
    {
        ...
        double err = sqrt(pow(X_0 - X_F1, 2) + pow(Y_0 - Y_F1, 2)) + sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2)) - 2 * sqrt(pow(b, 2) + pow(X_F1 - X_c, 2) + pow(Y_F1 - Y_c, 2));
        return scale * err;
    }
    ...
    {
        double deriv = 0.;
        if (param == p1x() || param == p1y() || param == f1x() || param == f1y() || param == cx() || param == cy() || param == rmin())
        {
            ...
            if (param == p1x())
            {
                deriv += (X_0 - X_F1) / sqrt(pow(X_0 - X_F1, 2) + pow(Y_0 - Y_F1, 2)) + (X_0 + X_F1 - 2 * X_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == p1y())
            {
                deriv += (Y_0 - Y_F1) / sqrt(pow(X_0 - X_F1, 2) + pow(Y_0 - Y_F1, 2)) + (Y_0 + Y_F1 - 2 * Y_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == f1x())
            {
                deriv += -(X_0 - X_F1) / sqrt(pow(X_0 - X_F1, 2) + pow(Y_0 - Y_F1, 2)) - 2 * (X_F1 - X_c) / sqrt(pow(b, 2) + pow(X_F1 - X_c, 2) + pow(Y_F1 - Y_c, 2)) + (X_0 + X_F1 - 2 * X_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == f1y())
            {
                deriv += -(Y_0 - Y_F1) / sqrt(pow(X_0 - X_F1, 2) + pow(Y_0 - Y_F1, 2)) - 2 * (Y_F1 - Y_c) / sqrt(pow(b, 2) + pow(X_F1 - X_c, 2) + pow(Y_F1 - Y_c, 2)) + (Y_0 + Y_F1 - 2 * Y_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == cx())
            {
                deriv += 2 * (X_F1 - X_c) / sqrt(pow(b, 2) + pow(X_F1 - X_c, 2) + pow(Y_F1 - Y_c, 2)) - 2 * (X_0 + X_F1 - 2 * X_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == cy())
            {
                deriv += 2 * (Y_F1 - Y_c) / sqrt(pow(b, 2) + pow(X_F1 - X_c, 2) + pow(Y_F1 - Y_c, 2)) - 2 * (Y_0 + Y_F1 - 2 * Y_c) / sqrt(pow(X_0 + X_F1 - 2 * X_c, 2) + pow(Y_0 + Y_F1 - 2 * Y_c, 2));
            }
            if (param == rmin())
            {
    ...
    void ConstraintCurveValue::errorgrad(double *err, double *grad, double *param)
    {
        ...
        if (this->pcoord() == this->p.x)
        { // this constraint is for X projection
            if (err)
            {
                *err = err_vec.x;
            }
        ...
        else if (this->pcoord() == this->p.y)
        { // this constraint is for Y projection
            if (err)
            {
                *err = err_vec.y;
            }
    ...
    double ConstraintMirrorPoint::grad(double *param)
    {
        ...
        if (param == pAx() || param == pAy() || param == pBx() || param == pBy() || param == axisP1x() || param == axisP1y() || param == axisP2x() || param == axisP2y())
        {
    ...
    double ConstraintCircularInstanceX::grad(double *param)
    {
        ...
        if (param == p0x() || param == p0y() || param == pkx() || param == pky() || param == cx() || param == cy() || param == angle())
        {
    ...
    ConstraintLinearInstanceX::ConstraintLinearInstanceX(Point &p0, Point &pk, Point &dirP1, Point &dirP2, double *base_distance, int N)
    {
        instanceN = N;
        pvec.push_back(p0.x);
        pvec.push_back(p0.y);
        pvec.push_back(pk.x);
        pvec.push_back(pk.y);
        pvec.push_back(dirP1.x);
        pvec.push_back(dirP1.y);
        pvec.push_back(dirP2.x);
        pvec.push_back(dirP2.y);
        pvec.push_back(base_distance);
        origpvec = pvec;
        rescale();
    }

    ConstraintType ConstraintLinearInstanceX::getTypeId()
    {
        return LinearInstanceX;
    }

    void ConstraintLinearInstanceX::rescale(double coef)
    {
        scale = coef * 1.;
    }

    double ConstraintLinearInstanceX::error()
    {
        double p0_x = *p0x(), p0_y = *p0y();
        double pk_x = *pkx();
        double dirP1_x = *dirP1x(), dirP1_y = *dirP1y();
        double dirP2_x = *dirP2x(), dirP2_y = *dirP2y();
        double base_dist = *base_distance();

        // Calculate direction vector
        double dx = dirP2_x - dirP1_x;
        double dy = dirP2_y - dirP1_y;
        double d2 = dx * dx + dy * dy;

        if (d2 < 1e-20)
        {
            // Direction vector is degenerate (zero length), return large error
            return scale * 1e10;
        }
        ...
        double actual_distance = base_dist * instanceN;

        // Compute translated point: pk_expected = p0 + (base_distance * N) * dir
        // X component: pk_x = p0_x + (base_distance * N) * dir_x
        double pk_x_expected = p0_x + actual_distance * dir_x;
    ...
    double ConstraintLinearInstanceX::grad(double *param)
    {
        double deriv = 0.;

        // Do not update axis or source points
        if (param == dirP1x() || param == dirP1y() || param == dirP2x() || param == dirP2y())
        {
            return 0.0;
        }
        ...
        if (param == p0x() || param == p0y() || param == pkx() || param == pky() ||
            param == dirP1x() || param == dirP1y() || param == dirP2x() || param == dirP2y() ||
            param == base_distance())
        {
            double p0_x = *p0x(), p0_y = *p0y();
            double dirP1_x = *dirP1x(), dirP1_y = *dirP1y();
            ...
            // Error: err = pk_x - [p0_x + (base_distance * N) * dir_x]
            // Derivatives:
            if (param == p0x())
            {
                // d/dp0_x: 0 (p0 should not change)
                deriv = 0.0;
            }
            else if (param == p0y())
            {
                // d/dp0_y: 0 (p0 should not change)
                deriv = 0.0;
            }
            else if (param == pkx())
            {
            ...
            else if (param == base_distance())
            {
                // d/d(base_distance): -N * dir_x
                deriv = -instanceN * dir_x;
            }
            else if (param == dirP1x())
            {
                // d/d(dirP1_x): derivative of -actual_distance * dir_x w.r.t. dirP1_x
                // dir_x = (dirP2_x - dirP1_x) / d = dx / d
                // d/d(dirP1_x) of dir_x = d/d(dirP1_x) of [dx / d]
                // Using quotient rule: (u/v)' = (u'v - uv')/v^2
                // u = dx, u' = -1
                // v = d, v' = d/d(dirP1_x) of d = -dx/d
                // So: d/d(dirP1_x) of dir_x = [(-1)*d - dx*(-dx/d)] / d^2 = [-d^2 + dx^2] / d^3 = -dy^2 / d^3
                // So d/d(dirP1_x) of err = actual_distance * dy^2 / d^3
                deriv = actual_distance * dy * dy / (d * d * d);
            }
            else if (param == dirP1y())
            {
                // d/d(dirP1_y): derivative w.r.t. dirP1_y
                // dir_x = dx / d
                // d/d(dirP1_y) of dir_x = d/d(dirP1_y) of [dx / d]
                // u = dx, u' = 0
                // v = d, v' = d/d(dirP1_y) of d = -dy/d
                // So: d/d(dirP1_y) of dir_x = [0*d - dx*(-dy/d)] / d^2 = dx*dy / d^3
                // So d/d(dirP1_y) of err = -actual_distance * dx * dy / d^3
                deriv = -actual_distance * dx * dy / (d * d * d);
            }
            else if (param == dirP2x())
            {
                // d/d(dirP2_x): derivative w.r.t. dirP2_x
                // dir_x = dx / d
                // d/d(dirP2_x) of dir_x = d/d(dirP2_x) of [dx / d]
                // u = dx, u' = 1
                // v = d, v' = d/d(dirP2_x) of d = dx/d
                // So: d/d(dirP2_x) of dir_x = [1*d - dx*(dx/d)] / d^2 = [d^2 - dx^2] / d^3 = dy^2 / d^3
                // So d/d(dirP2_x) of err = -actual_distance * dy^2 / d^3
                deriv = -actual_distance * dy * dy / (d * d * d);
            }
            else if (param == dirP2y())
            {
                // d/d(dirP2_y): derivative w.r.t. dirP2_y
                // dir_x = dx / d
                // d/d(dirP2_y) of dir_x = d/d(dirP2_y) of [dx / d]
                // u = dx, u' = 0
                // v = d, v' = d/d(dirP2_y) of d = dy/d
                // So: d/d(dirP2_y) of dir_x = [0*d - dx*(dy/d)] / d^2 = -dx*dy / d^3
                // So d/d(dirP2_y) of err = actual_distance * dx * dy / d^3
                deriv = actual_distance * dx * dy / (d * d * d);
            }
        }
        return scale * deriv;
    ...
    ConstraintLinearInstanceY::ConstraintLinearInstanceY(Point &p0, Point &pk, Point &dirP1, Point &dirP2, double *base_distance, int N)
    {
        instanceN = N;
        pvec.push_back(p0.x);
        pvec.push_back(p0.y);
    ...
    ConstraintType ConstraintLinearInstanceY::getTypeId()
    {
        return LinearInstanceY;
    }

    void ConstraintLinearInstanceY::rescale(double coef)
    {
        scale = coef * 1.;
    }

    double ConstraintLinearInstanceY::error()
    {
        double p0_x = *p0x(), p0_y = *p0y();
        double pk_y = *pky();
        double dirP1_x = *dirP1x(), dirP1_y = *dirP1y();
        ...
        // Compute translated point: pk_expected = p0 + (base_distance * N) * dir
        // Y component: pk_y = p0_y + (base_distance * N) * dir_y
        double pk_y_expected = p0_y + actual_distance * dir_y;
    ...
    double ConstraintLinearInstanceY::grad(double *param)
    {
        double deriv = 0.;
            ...
            param == base_distance())
        {
            ...
            // Error: err = pk_y - [p0_y + (base_distance * N) * dir_y]
            // Derivatives:
            if (param == p0x())
            {
                // d/dp0_x: 0 (p0 should not change)
            ...
            {
                // d/d(base_distance): -N * dir_y
                deriv = -instanceN * dir_y;
            }
            else if (param == dirP1x())
            {
                // d/d(dirP1_x): derivative w.r.t. dirP1_x
                // dir_y = (dirP2_y - dirP1_y) / d = dy / d
                // d/d(dirP1_x) of dir_y = d/d(dirP1_x) of [dy / d]
                // Using quotient rule: (u/v)' = (u'v - uv')/v^2
                // u = dy, u' = 0
                // v = d, v' = d/d(dirP1_x) of d = -dx/d
                // So: d/d(dirP1_x) of dir_y = [0*d - dy*(-dx/d)] / d^2 = dx*dy / d^3
                // So d/d(dirP1_x) of err = -actual_distance * dx * dy / d^3
                deriv = -actual_distance * dx * dy / (d * d * d);
            }
            else if (param == dirP1y())
            {
                // d/d(dirP1_y): derivative w.r.t. dirP1_y
                // dir_y = dy / d
                // d/d(dirP1_y) of dir_y = d/d(dirP1_y) of [dy / d]
                // u = dy, u' = -1
                // v = d, v' = d/d(dirP1_y) of d = -dy/d
                // So: d/d(dirP1_y) of dir_y = [(-1)*d - dy*(-dy/d)] / d^2 = [-d^2 + dy^2] / d^3 = -dx^2 / d^3
                // So d/d(dirP1_y) of err = actual_distance * dx^2 / d^3
                deriv = actual_distance * dx * dx / (d * d * d);
            }
            else if (param == dirP2x())
            {
                // d/d(dirP2_x): derivative w.r.t. dirP2_x
                // dir_y = dy / d
                // d/d(dirP2_x) of dir_y = d/d(dirP2_x) of [dy / d]
                // u = dy, u' = 0
                // v = d, v' = d/d(dirP2_x) of d = dx/d
                // So: d/d(dirP2_x) of dir_y = [0*d - dy*(dx/d)] / d^2 = -dx*dy / d^3
                // So d/d(dirP2_x) of err = actual_distance * dx * dy / d^3
                deriv = actual_distance * dx * dy / (d * d * d);
            }
            else if (param == dirP2y())
            {
                // d/d(dirP2_y): derivative w.r.t. dirP2_y
                // dir_y = dy / d
                // d/d(dirP2_y) of dir_y = d/d(dirP2_y) of [dy / d]
                // u = dy, u' = 1
                // v = d, v' = d/d(dirP2_y) of d = dy/d
                // So: d/d(dirP2_y) of dir_y = [1*d - dy*(dy/d)] / d^2 = [d^2 - dy^2] / d^3 = dx^2 / d^3
                // So d/d(dirP2_y) of err = -actual_distance * dx^2 / d^3
                deriv = -actual_distance * dx * dx / (d * d * d);
            }
        }
        return scale * deriv;
